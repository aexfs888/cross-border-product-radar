...
test('私密聚合合同只接受11国、匿名商品键和聚合指标', () => {
  ...
  const result = validatePrivateAggregateRows(headers, [{
    ...
    orders_count: '12', revenue_before_refunds_chargebacks: '1000', refunds_count: '1', refund_amount: '20',
...
test('私密聚合合同拒绝PII列、未配置国家和负金额', () => {
  ...
  const rowResult = validatePrivateAggregateRows(privateAggregateContract.requiredColumns, [{
    ...
    orders_count: '1', revenue_before_refunds_chargebacks: '-1', refunds_count: '0', refund_amount: '0',
  ...
  assert.ok(rowResult.errors.includes('row_2:revenue_before_refunds_chargebacks:invalid_nonnegative_number'))
})
