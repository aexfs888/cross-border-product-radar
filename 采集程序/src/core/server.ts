...
export function startDashboard(port = 8765): http.Server {
  const server = http.createServer((request, response) => {
    ...
    if (url.pathname === '/api/products') {
      ...
      try {
        json(response, 200, store.listProducts(bucket).map((product) => ({ id: product.id, name: product.zh_name || product.original_name, originalName: product.original_name, heat: product.research_heat_score, peakHeat: product.peak_heat_score, commercial: product.commercial_score, grade: product.commercial_grade, bucket: product.reuse_bucket, ageBand: product.trend_age_band, reason: product.restriction_reason || product.research_reason, updatedAt: product.updated_at })))
      } finally { store.close() }
      return
    }
